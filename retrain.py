...
def train_model(config, model, train_loader, test_loader, optimizer, device, num_epochs, eval_freq, eval_iter, start_context="Jack Gisburn rather a cheap genius- ", tokenizer=None, global_step=-1, checkpoint=None):
    total_loss = 0
    tokens_seen = 0
...
def load_checkpoint(model, optimizer, checkpoint_path):
    ...
    logger.info(f"Loaded checkpoint from step {start_step} with loss {loss}")
    return start_step, checkpoint

if __name__ == "__main__":
    # Add argument parsing
    parser = argparse.ArgumentParser(description='Train or continue training SmolLM model')
    ...
    if args.checkpoint:
        start_step, checkpoint = load_checkpoint(model, optimizer, args.checkpoint)
        global_step = start_step  # Update global_step in train_model
